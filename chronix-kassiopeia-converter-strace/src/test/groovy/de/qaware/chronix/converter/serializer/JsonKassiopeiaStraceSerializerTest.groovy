 ...
 */
class JsonKassiopeiaStraceSerializerTest extends Specification {

    def "test serialize to and deserialize from json"() {
        given:
        def times = longList([0, 1, 2])
        def values = ["string0", "string1", "string2"]
        def ts = new StraceTimeSeries.Builder("test").points(times, values).build()

        def serializer = new JsonKassiopeiaStraceSerializer()
        def start = 0l
        def end = 2l
        ...
        then:
        def builder = new StraceTimeSeries.Builder("test")
